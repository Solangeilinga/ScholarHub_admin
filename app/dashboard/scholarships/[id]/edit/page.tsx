...
export default function EditScholarshipPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await scholarshipApi.getOne(id as string)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('')
    try {
      await scholarshipApi.update(id as string, {
        ...form,
          ...
          <div className="flex flex-wrap gap-2">
            {LEVELS.map(l => (
              <button key={l} type="button" onClick={() => toggle('level', l)}
                className={`px-4 py-2 rounded-xl text-sm font-medium transition ${form.level.includes(l) ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                {l}
              </button>
            ))}
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            {COUNTRIES.map(c => (
              <button key={c} type="button" onClick={() => toggle('countries', c)}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${form.countries.includes(c) ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                {c}
              </button>
            ))}
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            {FIELDS.map(f => (
              <button key={f} type="button" onClick={() => toggle('fields', f)}
                className={`px-4 py-2 rounded-xl text-sm font-medium transition ${form.fields.includes(f) ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                {f}
              </button>
            ))}
          </div>
          ...
          <div className="flex gap-2">
            {LANGUAGES.map(l => (
              <button key={l} type="button" onClick={() => toggle('languages', l)}
                className={`px-4 py-2 rounded-xl text-sm font-medium transition ${form.languages.includes(l) ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                {l.toUpperCase()}
              </button>
            ))}
          </div>
